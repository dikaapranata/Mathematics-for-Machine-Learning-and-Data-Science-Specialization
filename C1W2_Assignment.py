...
def row_echelon_form(A, B):
    ...
    for row in range(num_rows):
        ...
        # Check if pivot_candidate is a valid number
        if isinstance(pivot_candidate, (int, float)) and not np.isnan(pivot_candidate):
...
def back_substitution(M):
    """
    Perform back substitution on an augmented matrix (with unique solution) in reduced row echelon form
    to find the solution to the linear system.
    ...
    for row in range(num_rows - 1, -1, -1):
        ...
        if pivot_index is None:
            raise ValueError("Invalid augmented matrix provided")

        solution[pivot_index] = M[row, -1]
        for j in range(row + 1, num_rows):
            solution[pivot_index] -= M[row, j] * solution[j]
...
def gaussian_elimination(A, B):
    ...
    # Combine matrices A and B into an augmented matrix
    ...
    if not isinstance(row_echelon_M, str):
        ...
        # If row_echelon_M is a string, it indicates either a singular system or no solution
