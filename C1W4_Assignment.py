...
# Define the corrected transition matrix P.
P = np.array([
    [0, 0.15, 0.15, 0.15, 0.55],
    [0.15, 0, 0.15, 0.05, 0.05],
    [0.15, 0.35, 0, 0.05, 0.25],
    [0.25, 0.25, 0.25, 0, 0.25],
    [0.45, 0.25, 0.45, 0.75, 0]
]) 

# Define the initial state vector X0.
X0 = np.array([[0], [0], [0], [1], [0]])

# Multiply matrix P and X0 (matrix multiplication).
X1 = np.dot(P, X0)
...
def check_eigenvector(P, X_inf):
    # Perform matrix multiplication P * X_inf
...
def center_data(Y):
    ...
    # Repeat the mean values for each observation
    mean_matrix = np.repeat(mean_vector, Y.shape[0], axis=0)
    
    # Reshape the mean matrix to have the same shape as the original data
    mean_matrix = np.reshape(mean_matrix, Y.shape, order='F')
    
    # Subtract the mean matrix from the original data to center it
    X = Y - mean_matrix
...
def perform_PCA(X, eigenvecs, k):
    ...
    Returns:
        Xred (ndarray): reduced data matrix. Has dimensions (n_observations)x(k)
    """
    # Extract the first k eigenvectors
    V = eigenvecs[:, :k]
    
    # Perform dimensionality reduction by taking the dot product
    Xred = np.dot(X, V)
